...
Compose 会同时启动前端和 API 服务，用户账号、设置和渠道密钥保存在 `app-data` 卷中。完整构建使用 `docker compose up -d --build`。如需更换宿主机端口，将 `ports` 左侧改为例如 `8080:3000` 后重新执行命令。

## AI 服务地址

主应用通过 `AI_BASE_URL` 运行期变量锁定所有渠道的接口地址，默认为 `https://api.smartxai.cn`。前端渠道配置中只读展示该地址，用户无法修改。

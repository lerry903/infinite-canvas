...
+ [调整] AI 服务接口地址改由部署配置统一锁定，同时禁用配置导入并移除本地代理页签。
+ [调整] 移除普通页面和画布右上角的 GitHub 图标及仓库跳转入口。
+ [新增] 配置存储增加当前用户作用域，用户切换后渠道、API Key 和 WebDAV 偏好不再共用同一份本地记录。
+ [新增] 增加可选的 `/api/session` 与 `/api/me/settings` 会话和用户设置接口边界，未接入后端时继续使用匿名本地模式。
